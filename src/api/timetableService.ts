...
export const generateTimetable = async (payload: {
  ...
  const response = await axios.post(`${API_BASE}/generate`, payload, authHeaders());
...
export const deleteTimetable = async (id: number | string) => {
  const response = await axios.delete(`${API_BASE}/${id}`, authHeaders());
  return response.data;
};

export const fetchTeacherTimetable = async () => {
  const response = await axios.get(`${API_BASE}/teacher/me`, authHeaders());
  return response.data as { success: boolean; data: Timetable | null };
};

export const fetchStudentTimetable = async () => {
  const response = await axios.get(`${API_BASE}/student/me`, authHeaders());
  return response.data as { success: boolean; data: Timetable | null };
};

export const fetchTimetableSlots = async (timetableId: number | string) => {
  const response = await axios.get(`${API_BASE}/${timetableId}/slots`, authHeaders());
