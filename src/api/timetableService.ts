...
export const createTimetableSlot = async (
  ...
  payload: Required<Pick<ManualTimetableSlotPayload, 'dayOfWeek' | 'slotIndex' | 'subjectId' | 'teacherId'>> & ManualTimetableSlotPayload
) => {
  const response = await axios.post(
    `${API_BASE}/${timetableId}/slots`,
    payload,
    authHeaders('admin')
  );
  return response.data;
...
export const updateTimetableSlot = async (
  timetableId: number | string,
  slotId: string,
  payload: ManualTimetableSlotPayload
) => {
  const response = await axios.patch(
    `${API_BASE}/${timetableId}/slots/${slotId}`,
    payload,
    authHeaders('admin')
  );
  return response.data;
...
export const removeTimetableSlot = async (timetableId: number | string, slotId: string) => {
